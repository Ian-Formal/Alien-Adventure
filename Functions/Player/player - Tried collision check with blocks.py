...
class L_Player(Player):
    ...
    def normal_move(self):
        ...
        if self.can_climb or self.can_climb is None:
            up = keys_dict[keys[py.K_UP] or keys[py.K_w]]
            down = keys_dict[keys[py.K_DOWN] or keys[py.K_d]]
            self.climb(up - down)
        elif self.climbing:
            self.climbing = False
    ...
    def move_x(self, x=None):
        ...
        if x is None:
            ...
            self.x += x
        y = self.y
        self.handle_collision_in_dir(0, 1)
        if self.y < y - abs(self.x_vel) - 4:
            self.y = y
    ...
    def check_move_x(self) -> bool:
        ...
        if check_x:
            print(self.pins['right'], self.collided[0].x + self.x_vel + 0.000001, self.pins['left'], self.collided[0].x + self.collided[0].width + self.x_vel - 0.000001)
        return check_x
    ...
    def handle_collision_in_dir(self, dx, dy):
        ...
        # Collision is tested twice for little slope blocks, getting out of the collision
        collided = False
        for _ in range(2):
    ...
    def handle_moving_collision(self, block, dx, dy, fix_dx, fix_dy, feet):
        """Handles collisions with movable tiles"""
        grass_block = self.collided[block].__class__.__name__ == 'Grass_Block'
        if grass_block:
            if (dy - self.collided[block].y) - fix_dy > 1 or not feet or self.climbing:
                return False

        if fix_dy > 0:
            self.y = self.collided[block].rect.top - self.rect.height + 2.000001
        elif fix_dy < 0:
            self.y = self.collided[block].rect.bottom + 0.000001
        if fix_dx != 0:
            if self.collided[block].x > self.x:
                self.x = self.collided[block].rect.left - self.rect.width - 0.000001
            else:
                self.x = self.collided[block].rect.right + 0.000001

        if fix_dy != 0:
            self.y += self.camera_y + self.tile_size
        if fix_dx != 0:
        ...
        if fix_dy > 0:
            if not feet:
                ...
                self.y -= self.collided[block].rect.top - self.height - dy
                self.y = round(self.y)
                self.y += self.collided[block].y_vel + 0.1
        elif fix_dy < 0:
            self.y += dy - self.collided[block].rect.bottom + 1.000001
        if fix_dx < 0:
            self.x += dx - self.collided[block].rect.right + 1.000001
        elif fix_dx > 0:
            self.x -= self.collided[block].rect.left - self.width - dx"""
        return True

    def handle_collision_at_point(self, x, y, fix_dx, fix_dy, feet=False):
        """Handles collision at points of the player"""
        collision = False
        for i, collided in enumerate(self.collided):
            if collided.rect.collidepoint(x - self.camera_x, y - self.camera_y):
                dx, dy = x - self.camera_x, y - self.camera_y
                collision = collision or self.handle_moving_collision(i, x, y, fix_dx, fix_dy, feet)
