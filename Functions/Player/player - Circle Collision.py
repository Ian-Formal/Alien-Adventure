...
class L_Player(Player):
    ...
    def handle_collision(self):
        ...
        self.move_y()
        if self.collided:
            self.handle_moving_collision()
        self.update_draw_pos()

    def handle_collision_in_dir(self, dx, dy):
        """Handles collision in a direction"""
        # collided follows: top-left, top-right, bottom-left, bottom-right, middle-left, middle-right
        self.update_pins()
        ...
        collided = self.handle_collision_at_point(left, top, dx, dy) or \
            ...
            self.handle_collision_at_point(left, mid, dx, dy) or \
            self.handle_collision_at_point(right, mid, dx, dy)

        return collided

    def handle_moving_collision(self):
        """Handles collisions with movable tiles"""
        def check_touching_solids():
            """Checks if player is colliding with any object"""
            collided = self.handle_collision_in_dir(0, 0)
            return collided or py.sprite.collide_rect(self, self.collided)
        distance = 1
        direction = 0
        ...
        for _ in range(32):
            for _ in range(16):
                ...
                self.x += distance * cos(radians(direction))
                self.y -= distance * sin(radians(direction))
                self.update_draw_pos()
                self.rect = py.transform.scale(self.image[0], (self.scale * 70, self.scale * 100)) \
                    .get_rect(topleft=(self.draw_x, self.draw_y - (-20 + self.height) * self.scale))
                self.mask = py.mask.from_surface(py.transform.scale(self.image[0], (self.scale * 70, self.scale * 100)))
                if not check_touching_solids():
                    self.collided = False
                    return None
                direction += 360 / 16
            direction = 0
