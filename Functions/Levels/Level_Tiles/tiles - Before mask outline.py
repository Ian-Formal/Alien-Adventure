...
class Tile(py.sprite.Sprite):
    ...
    def find_collision(self):
        ...
        if '_' in self.collision or '=' in self.collision:
            x, y = 1, self.rect.height - (31 if '_' in self.collision else 1)
            while self.mask.get_at((x, y)) == 0:
                y -= (-0.1 if '_' in self.collision else 0.1)
            self.col_height = round(y * (-1 if '_' in self.collision else 1), 3)
        elif '>' in self.collision or '<' in self.collision:
            x, y = self.rect.width - (1 if '<' in self.collision else 31), 1
            while self.mask.get_at((x, y)) == 0:
                x += -0.1 if '<' in self.collision else 0.1
            self.col_width = round(x * (1 if '<' in self.collision else -1), 3)
...
class Tiles:
    ...
    def setup(self, grid_size: tuple, setup_dict=None):
        ...
        self.tile_index = 0
        self.tiles[-1] = Tile(self.tile_class, 0, 0, -1, -1)
    ...
    def check_off_screen(self, tile):
        ...
        # This is the other way round because pygame is weird and y values ascend as it goes down
        if self.tiles[i].y - self.camera_y > self.sc_height:
            ...
            self.loop_tile_y(i, self.tile_count_y)
        # I think I switched these around to make them work, I will take it. (Spent 1hr on this)
