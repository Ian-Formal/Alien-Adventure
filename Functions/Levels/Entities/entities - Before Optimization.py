...
class All_Entities:
    ...
    __slots__ = 'screen', 'sc_width', 'sc_height', 'player', 'editor', 'hud', 'entity_size', 'path', 'camera_x', \
                'camera_y', 'grid_list', 'grid_list_info', 'entities', 'entity_dict', 'door_state', 'total_doors', \
                'key_cls'
    ...
    def move_entity(self):
        ...
        for entity in self.entities:
            ...
            # Movable tiles mainloop
            if not hasattr(self.entities[entity], 'mov_dir'):
                continue
        ...
        # Others
        for entity in self.entities:
            if not self.entities[entity].show_last:
                continue
            if hasattr(self.entities[entity], 'mov_dir'):
                continue
            self.entities[entity].update_draw_pos()
            self.entities[entity].mainloop()
        for entity in self.entities:
            if self.entities[entity].show_last:
                continue
            if hasattr(self.entities[entity], 'mov_dir'):
                continue
            self.entities[entity].update_draw_pos()
        ...
        for entity in self.entities:
            if not hasattr(self.entities[entity], 'mov_dir'):
                continue
            if floor(self.entities[entity].group) not in Movable_Tile.group_touching:
                continue
