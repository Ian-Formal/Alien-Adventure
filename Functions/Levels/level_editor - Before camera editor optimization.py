...
class Camera_Rect(py.sprite.Sprite):
    ...
    def move_camera(self, *, cam_x, cam_y, player_cls):
    ...
    def free_camera_rect(self, cam_x, cam_y, player_cls):
        ...
        if 'x' not in self.lock:
            ...
            if player_cls.draw_x > self.loc_range.right:
                cam_x += player_cls.draw_x - self.loc_range.right
            elif player_cls.draw_x < self.loc_range.left:
                cam_x += player_cls.draw_x - self.loc_range.left
        if 'y' not in self.lock:
            cam_y += (player_cls.y - cam_y - self.sc_height // 2) / 8
        elif not self.in_loc_range(player_cls.draw_x, player_cls.draw_y):
            if player_cls.draw_y > self.loc_range.bottom:
                cam_y += player_cls.draw_y - self.loc_range.bottom
            elif player_cls.draw_y < self.loc_range.top:
                cam_y += player_cls.draw_y - self.loc_range.top
    ...
    def draw(self):
        ...
        surface = py.Surface((self.rect.width, self.rect.height))
        surface.fill(col)
        surface.set_alpha(100)
        self.screen.blit(surface, (self.left - self.camera_x, self.top - self.camera_y))
...
class Editor(Level):
    ...
    __slots__ = 'tiles', 'entity', 'editor', 'editor_exit_func', 'esc_pressed', 'del_pressed', 'tab_pressed', \
                'clicked', 'buttons', 'show_menu', 'full_screen_menu', 'door_placing', 'ent_menu', 'user_text',\
                'user_text_length', 'l2', 'rect_pos', 'camera_menu', 'top_menu', 'side_menu', 'selected_tile', 'font', \
                'button_pressed', 'full_menu_font', 'small_menu_font', 'full_menu_bg', 'menu_buttons', 'tile_y', \
                'max_tile_y', 'tile_y_vel', 'chosen_brush', 't1', 't2', 't3', 't4', 't5', 't6', 't7', 't8', 't9', 't0',\
                'img_dict', 'once_tiles', 'tile_images', 'tile_font', 'editor_permission', 'current_text', \
                'clearing_level', 'n_grid_dims', 'setup_func', 'grid_func', 'player_move'
    ...
    def tiles_menu_mainloop(self):
        ...
        self.tile_y_vel += move_dict[keys[py.K_DOWN]] - move_dict[keys[py.K_UP]]
        if abs(self.tile_y_vel) > 20:
            self.tile_y += self.tile_y_vel / abs(self.tile_y_vel) * 30
        elif abs(self.tile_y_vel) > 0:
            self.tile_y += self.tile_y_vel / abs(self.tile_y_vel) * 3
        if move_dict[keys[py.K_DOWN]] - move_dict[keys[py.K_UP]] == 0:
            self.tile_y_vel = 0
    ...
    def get_tile_at_pos(self, x, y):
        ...
        # OMG I FORGOT AGAIN THAT Y POS GOES TO NEGATIVE AS IT INCREASES (Spent 3hrs)
        tile_index = tile_grid_y + tile_grid_x * self.tiles.grid_height
